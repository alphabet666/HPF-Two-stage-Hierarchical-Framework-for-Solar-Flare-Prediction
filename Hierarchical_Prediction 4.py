...
class BASE_MODEL_BOOSTING(object):
    ...
    def train(self):
        ...
        for i in range(len(self.classifier_lis)):
            ...
            print(str(i), str(model)[:10], f1, f05,reca, prec, accuracy, Hss, Tss)
            #print(str(i), str(model)[:10], f1, f05,reca, prec, accuracy)
            a = [str(model)[:10],f1,f05,reca,prec,accuracy, Hss, Tss]
            csv1.append(a)

        with open('HPF_base_output_partition7.csv', 'w', newline='') as file:
    ...
    # 逐行写入数据
        ...
        print("Data has been written to HPF_base_output_partition7.csv")
